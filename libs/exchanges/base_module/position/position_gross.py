...
class OpenPositionGross(object):
    ...
    def renew_posfile(self, filename):
        ...
        for key,val in self._long_position.items():
            self._update_posfile(val)
        for key,val in self._short_position.items():
            self._update_posfile(val)
                ...
                if 'position' in data :
                    ...
                    if d['size']==0 :
                        if d['posid'] in position_dict:
                            ...
                            self.logger.error( "ID {} is not in position list".format(d['posid']))
                    else:
    ...
    def execution(self, posid, side, price, size, orderid, commission=0, settleType='OPEN'):
        assert size > 0
        org_side = self.side
        ...
        if settleType=='OPEN' :
            ...
            if side=='BUY' :
                if posid in self._short_position :
                    ...
                    # 決済オーダー出していないのに決済オーダーは来るはずはない
#                    if not self._short_position[posid]['closeorder']:
#                        self._logger.error("Close position error!!!  [pos={}]".format(self._short_position[posid]))
                ...
                if posid in self._long_position :
                    ...
                    # 決済オーダー出していないのに決済オーダーは来るはずはない
#                    if not self._long_position[posid]['closeorder']:
#                        self._logger.error("Close position error!!!  [pos={}]".format(self._long_position[posid]))
