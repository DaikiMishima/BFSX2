    ...
    def initialize(self):
        ...
        for exchange, symbol in self.parameters['exchanges']:
            ...
            if exchange == 'bitFlyer' :
                ...
                websocket = BitflyerWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'binance' :
                from libs.exchanges.binance import BinanceWebsocket
                websocket = BinanceWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'phemex' :
                from libs.exchanges.phemex import PhemexWebsocket
                websocket = PhemexWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'bybit' :
                from libs.exchanges.bybit import BybitWebsocket
                websocket = BybitWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'GMO' :
                from libs.exchanges.gmo import GmoWebsocket
                websocket = GmoWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'bitmex' :
                from libs.exchanges.bitmex import BitmexWebsocket
                websocket = BitmexWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'coinbase' :
                from libs.exchanges.coinbase import CoinbaseWebsocket
                websocket = CoinbaseWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'deribit' :
                from libs.exchanges.deribit import DeribitWebsocket
                websocket = DeribitWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'FTX' :
                from libs.exchanges.ftx import FtxWebsocket
                websocket = FtxWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'huobi' :
                from libs.exchanges.huobi import HuobiWebsocket
                websocket = HuobiWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'kraken' :
                from libs.exchanges.kraken import KrakenWebsocket
                websocket = KrakenWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'okex' :
                from libs.exchanges.okex import OkexWebsocket
                websocket = OkexWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            elif exchange == 'bitfinex' :
                from libs.exchanges.bitfinex import BitfinexWebsocket
                websocket = BitfinexWebsocket(self._logger, subscribe={'board': True}, symbol=symbol)

            else:
                self._logger.info( "Not supported : {}".format(exchange) )
                continue

            board_list = self.BoardQueue(callback=self.board, ws=websocket)
            self.exchanges.append( { 'websocket': websocket, 'name':websocket.__class__.__name__[:-9],
                                     'board_list': board_list, 'symbol': symbol.replace('/','-'), 'counter':0 } )
    ...
    def board(self):
        ...
        with self.lock:
            ...
            for exchange in self.exchanges:
                ...
                while len(exchange['board_list'])!=0:
                    i = exchange['board_list'].popleft()
                    logger.info("{},{},{},{}".format(i['exec_date'], i['side'], i['price'], i['size']))
                    exchange['counter'] +=1
