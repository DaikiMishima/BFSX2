...
#        for key,val in self._profit_history.items():
#            print( "-------------\n", key , "\n-----------------" )
#            for v in val:
#                print( v )
        ...
        # Ctrl+Cで現物のスレッドを停止させるために登録しておく
        self._stop = False
        self._logger.stop_list.append([self.stop, self.realtime_logic])
        ...
        self.order_signal_event = Event()
        Thread(target=self._wait_event).start()
    ...
    def parse_trade_signal(self, trade_signal):
        ...
        if len(signal_message) == 5:  # はむとれのシグナルはカンマ区切りで５個
            ...
            self.handle_trade_signal(signal_logic_name, signal_order_side, signal_order_size, signal_order_comment)
    ...
    def realtime_logic(self):
        ...
        while len([o['time'] for o in list(self._order_list) if o['time'] < time.time()]) != 0:
            ...
            if self.no_trade_period:
                response = "No trade period. Pending : [{}]logic : side:{} size:{} comment:{}".format(order['logic'], order['side'],  order['size'], order['comment'])
            else:
                response = self.sendorder(order_type='MARKET', side=order['side'], size=order['size'])  # オーダー発出
    ...
    def loss_cut_check(self):
        ...
        if len(self._ordered_list.keys()) != 0:
            ...
            for key, value in order_list.items():
                ...
                total_size = round(sum([e['size'] for e in executed if e['id'] == key]), 8)

                # すべて約定していたら
                if total_size >= round(value['size'], 8):
                    # 平均約定価格を計算
                    average_price = round(sum([e['size']*e['price'] for e in executed if e['id'] == key])/total_size, 1)

                    # オーダーリストから削除
                    del self._ordered_list[key]

                    message = "Filled id: {} size:{} average:{:.1f}  [{}]".format(key, round(value['size'], 8), average_price, value['logic'])
                ...
                # 部分約定
                    ...
                    self._logger.info("Partially filled id: {} size:{} [{}]".format(key, total_size, value['logic']))
    ...
    def _append_to_profit_history(self, data):
        try:
            dt, logic, id, size, price = data['time'], data['logic'], data['id'], float(data['size']), float(data['price'])

            if self._profit_history_first_dt == 0:
                self._profit_history_first_dt = dt

            # リニア契約かインバース契約か
            unitrate = price if self.ws.units()['unitrate'] == 1 else (1/price)
            if id and logic :
                # ノーポジからの新規ポジ構築
                if self._profit_history.get(logic) == None:
                    if self._plot_start_offset.get(logic) == None:
                        self._profit_history[logic] = [{'time': dt, 'pos': size, 'exec': size*unitrate, 'ltp': price, 'pnl': 0}]
                    else:
                        self._profit_history[logic] = [{'time': dt, 'pos': size+self._plot_start_offset[logic],
                                                        'exec':(size+self._plot_start_offset[logic])*unitrate, 'ltp':price, 'pnl':0}]
#                    print( "1:" + logic,self._profit_history[logic][-1] )
                ...
                # すでにポジ情報がある場合にはポジの増減
                    ...
                    new_exec = previous['exec']+size*unitrate
                    self._profit_history[logic].append({'time': dt, 'pos': new_size, 'exec': new_exec, 'ltp': price, 'pnl': new_exec-(new_size*unitrate)})
#                    print( "2:" + logic,self._profit_history[logic][-1] )
            ...
            # 定期的に記録されるLTP情報によって含み損益を再計算
                ...
                for key, value in self._profit_history.items():
                    previous = value[-1]
                    value.append({'time': dt, 'pos': previous['pos'], 'exec': previous['exec'], 'ltp': price, 'pnl': previous['exec']-(previous['pos']*unitrate)})
#                    print( "3:" + key,value[-1] )
    ...
    def _plot_profit_graph(self, logic=None, top=-1, bottom=-1, start_datetime=datetime(1970, 4, 27), endt_datetime=datetime(2100, 12, 31)):
        ...
        try:
            ...
            for key, history in self._profit_history.items():
                ...
                if len(value) != 0:
                    pnl = [(v['pnl']-value[0]['pnl'])*(1 if self.ws.units()['unitrate'] == 1 else v['ltp']) for v in value]
                    timescale = [_epoc_to_dt(v['time']) for v in value]
                    total_profit_list += [{'time': v['time'], 'pnl':(v['pnl'] - value[0]['pnl'])*(1 if self.ws.units()['unitrate'] == 1 else v['ltp']), 'logic':key, 'ltp':v['ltp']} for v in value]
                    plot_list.append({'pnl': (value[-1]['pnl']-value[0]['pnl'])*(1 if self.ws.units()['unitrate'] == 1 else value[-1]['ltp']), 'logic': key, 'plotx': timescale, 'ploty': pnl})
