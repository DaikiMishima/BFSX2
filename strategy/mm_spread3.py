...
class MyStrategy(Strategy):
    ...
    def loss_cut_check(self):
        ...
        # オーダーリストを表示（IDのみ抜き出して表示）
        self._logger.info( "    open orders : {}".format([o['id'] for o in ordered_list]) )

        # max_order_count よりも多いオーダー数があれば、古い物からキャンセル
        for i in range(-len(ordered_list), -self.parameters['max_order_count'], 1):
            self.cancelorder( ordered_list[i]['id'] )
